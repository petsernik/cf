...
private:
    struct node {
        ...
        node(T x = T()) : sum(x), inheritance(0), indicator(0) {}
        // merge nodes
        ...
        void update(const node &parent, int l, int r) {
            if (!parent.indicator) add(parent.inheritance, l, r);
            else change(parent.inheritance, l, r);
        }
        ostream &operator <<(ostream &ostr) {
            return ostr << sum << " " << inheritance << " " << indicator << "\n";
        }
    };
    vector<node> tree;
    int n;
